...
//        String bb = jedis.get("sku-101");
        ...
        String lockKey = null;
        if(!StrUtil.isBlank(value) && value.equals(userCode)){//加锁成功后查询库存
            try {
                //synchronized (key){
                lockKey = key;
                ...
                if( stock >= num){
                    ...
                    Object eval = jedis.eval(script, CollectionUtil.newArrayList(STOCK_KEY_PREFIX+lockKey), CollectionUtil.newArrayList(String.valueOf(0-num)));
